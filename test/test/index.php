...
$drama_name_eng="Rookies";
include('../../drama_contents.php');
?>


<?echo "below is test/test/test";?>
<table width="800" id="rounded-corner" summary="<?echo $drama_name_eng;?> Episode Chart">
    ...
    <tbody>
		<?for ($i=1;$i<=3;$i++){
			...
			$result1 = mysql_query("SELECT * FROM episodes WHERE drama_name_eng='$drama_name_eng' AND eps_type='$eps_type' ORDER BY eps_num");
			if ($result1) {
				while ($eps_row = mysql_fetch_array($result1)) {
					?><tr>	<td align="center">
					<?$result2 = mysql_query("SELECT * FROM episode_parts WHERE drama_name_eng='$drama_name_eng' AND eps_type='$eps_type' AND eps_num='$eps_row[eps_num]' ORDER BY part_num");
					...
					if ($result2) { 
						...
						?><a href="<?echo $episode_php_path;?>"><b><?echo $eps_row[eps_num];?></b></a></td>
						<td align="left">
						<a href="<?echo $episode_php_path;?>"><b><?
							if (strlen($eps_row[eps_title])==0) {echo ucwords($drama_name_eng." ".$eps_row[eps_type])." ".$eps_row[eps_num];}
							else {echo $eps_row[eps_title];}?></b></a><?
					}
					?></td>
					<td align="left">
					<?$result2 = mysql_query("SELECT * FROM episode_parts WHERE drama_name_eng='$drama_name_eng' AND eps_type='$eps_type' AND eps_num='$eps_row[eps_num]' ORDER BY part_num");
...
<table id="rounded-corner" summary="2007 Major IT Companies' Profit">
    ...
    <tfoot>
    	<tr>
        	<th scope="col" class="rounded-foot-left">Company</th>
			<th scope="col">Q1</th>
            <th scope="col">Q2</th>
            <th scope="col">Q3</th>
        	<th scope="col" class="rounded-foot-right">Q4</th>
        </tr>
    </tfoot>
    <tbody>
    	<tr>
