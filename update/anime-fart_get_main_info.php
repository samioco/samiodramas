<?php
if (!$a_f_file = file_get_contents($a_f_page)) {echo "Anime-Fart page not found.";?><br /><?}
else {
	echo "Anime-Fart page found. Retrieving basic information...";}?><br /><?

/*
$series_name_start_str = "<title>";
$series_name_end_str = " - DramaWiki</title>";
$offset = strlen($series_name_start_str);
$wiki_name_startpos = stripos($a-f_file, $series_name_start_str) + $offset; 
$wiki_name_endpos = stripos($a-f_file, $series_name_end_str, $wiki_name_startpos);
$series_name_eng = substr($a-f_file, $wiki_name_startpos, $wiki_name_endpos - $wiki_name_startpos);
...
$asian_name_startpos = stripos($a-f_file, $asian_name_start_str1) + $offset1;
$asian_name_startpos = stripos($a-f_file, $asian_name_start_str2, $asian_name_startpos) + $offset2;
$asian_name_endpos = stripos($a-f_file, $asian_name_end_str, $asian_name_startpos);
$series_name_asian = substr($a-f_file, $asian_name_startpos, $asian_name_endpos - $asian_name_startpos);
...
$network_startpos = stripos($a-f_file, $network_start_str) + $offset;
$network_endpos = stripos($a-f_file, $network_end_str, $network_startpos);
$network = substr($a-f_file, $network_startpos, $network_endpos - $network_startpos);
...
$broadcast_startdate_startpos = stripos($a-f_file, $broadcast_start_str) + $startdate_offset;
$broadcast_startdate_endpos = stripos($a-f_file, $broadcast_end_str, $broadcast_startdate_startpos);
$start_date = substr($a-f_file, $broadcast_startdate_startpos, $broadcast_startdate_endpos - $broadcast_startdate_startpos);
...
$broadcast_enddate_endpos = stripos($a-f_file, $broadcast_final_str, $broadcast_enddate_startpos);
$end_date = substr($a-f_file, $broadcast_enddate_startpos, $broadcast_enddate_endpos - $broadcast_enddate_startpos);
...
$rating_startpos = stripos($a-f_file, $rating_start_str);

if ($rating_startpos>0){
	$rating_startpos += $rating_offset;
	$rating_endpos = stripos($a-f_file, $rating_end_str, $rating_startpos);
	$rating = substr($a-f_file, $rating_startpos, $rating_endpos - $rating_startpos);
	$rating = trim($rating);
...
$site_startpos = stripos($a-f_file, $site_start_str);
if ($site_startpos>0) {
	$site_startpos += $site_offset;
	$site_endpos = stripos($a-f_file, $site_end_str, $site_startpos);
	$site = substr($a-f_file, $site_startpos, $site_endpos - $site_startpos);
...
$syn_startpos = stripos($a-f_file, $synopsis_start_str1);
$syn_startpos = stripos($a-f_file, $synopsis_start_str2, $syn_startpos) + $syn_offset;
$syn_endpos = stripos($a-f_file, $synopsis_end_str, $syn_startpos);
$synopsis = substr($a-f_file, $syn_startpos, $syn_endpos - $syn_startpos);
