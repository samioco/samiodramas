<?php
	...
	$cast_name_array = str_ireplace("\\","",$_POST["cast_name"]); 
	$cast_role_array = str_ireplace("\\","",$_POST["cast_role"]);
	$cast_detail_array = str_ireplace("\\","",$_POST["cast_detail"]);
	...
	while ($cast_name_array[$i]) {
		if ($delete_array[$i]!="DELETE"){ 
			...
			echo "\"".$cast_name_array[$i]."\",\"".$series_name_eng."\",\"".$cast_role_array[$i]."\",\"".$cast_detail_array[$i]."\",\"".$main_cast_array[$i]."\");";	
			echo "\r\n";
		}
		$i++;
	}?></textarea></td>
	...
	for ($i=1;$i<=$add_rows;$i++) {
		...
		<td><input type="text" size="25" name="cast_role[<?echo $row;?>]" value="actor"></td>
		<td><input type="text" size="50" name="cast_detail[<?echo $row;?>]" value=""></td>
		<td><input type="text" size="3" maxlength="1" name="main_cast[<?echo $row;?>]" value="0"></td>
		</tr><?
		$row++;
	}
	...
	<td colspan="6" width="100%">Add <input type="text" size="4" maxlength="4" name="add_rows" value="0"> more rows.</td></tr>
